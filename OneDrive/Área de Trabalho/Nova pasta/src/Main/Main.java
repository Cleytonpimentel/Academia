...
public class Main {
    ...
    public static void main(String[] args) {
        while (true) {
            ...
            System.out.println("5. Gerenciar Ficha Treino");
        ...
        switch (opcao) {
            ...
            case 4:
                ...
                if (planoParaRemover != null) {
                    // Agora, chame o método correto para remover o plano
                    planoDAO.removerPlano(planoIdRemover); // Removendo o plano
    ...
    private static void gerenciarFichaTreino() {
    	
        FichaTreinoDAO fichaTreinoDAO = new FichaTreinoDAO();

        System.out.println("Gerenciar Fichas de Treino:");
        System.out.println("1. Cadastrar Ficha de Treino");
        System.out.println("2. Buscar Ficha de Treino");
        System.out.println("3. Atualizar Ficha de Treino");
        System.out.println("4. Remover Ficha de Treino");
        ...
        switch (opcao) {
        case 1:
            // Cadastrar Ficha de Treino
            System.out.println("Cadastro de Ficha de Treino:");
            System.out.print("Membro ID: ");
            int membroId = scanner.nextInt();
            scanner.nextLine();  // Limpar o buffer
            Membro membro = membroDAO.buscarMembroPorId(membroId);

            if (membro != null) {
                System.out.print("Instrutor ID: ");
                int instrutorId = scanner.nextInt();
                scanner.nextLine();  // Limpar o buffer
                Instrutor instrutor = instrutorService.buscarInstrutorPorId(instrutorId);
                ...
                if (instrutor != null) {
                    ...
                    // Criando a Ficha de Treino
                    // Usando o construtor que aceita apenas membro, instrutor, descrição, dataInicio e dataFim
                    FichaTreino fichaTreino = new FichaTreino(membro, instrutor, "Descrição da ficha", dataInicio, dataFim);
            ...
            case 2:
                ...
                System.out.print("Digite o ID da Ficha de Treino: ");
                int idFichaTreino = scanner.nextInt();
                scanner.nextLine();  // Limpar o buffer

                // Buscar a ficha de treino no banco de dados
                FichaTreino fichaTreinoBuscada = fichaTreinoDAO.buscarFichaTreinoPorId(idFichaTreino);

                if (fichaTreinoBuscada != null) {
                    System.out.println("ID: " + fichaTreinoBuscada.getFichaid()); // Correção para getFichaid
                    System.out.println("Membro: " + fichaTreinoBuscada.getMembro().getNome());
                    System.out.println("Instrutor: " + fichaTreinoBuscada.getInstrutor().getNome());
                    System.out.println("Data de Início: " + fichaTreinoBuscada.getDataInicio());
                    System.out.println("Data de Fim: " + fichaTreinoBuscada.getDataFim());

                    // Agora, buscar as atividades associadas à ficha de treino
                    AtividadeDAO atividadeDAO = new AtividadeDAO();
                    List<Atividade> atividades = atividadeDAO.getAtividade();  // Instanciando AtividadeDAO corretamente

                    if (!atividades.isEmpty()) {
                        System.out.println("=== Atividades Associadas ===");
                        for (Atividade atividade : atividades) {
                            // Exibir informações de atividade
            ...
            case 3:
                ...
                System.out.print("Digite o ID da Ficha de Treino para atualizar: ");
                idFichaTreino = scanner.nextInt();
                scanner.nextLine();  // Limpar o buffer

                FichaTreino fichaParaAtualizar = fichaTreinoDAO.buscarFichaTreinoPorId(idFichaTreino);

                if (fichaParaAtualizar != null) {
                    System.out.println("Ficha de treino encontrada.");

                    // Atualizar data de início e fim
                    System.out.print("Novo Data de Início (yyyy-MM-dd): ");
                    String novaDataInicio = scanner.nextLine();
                    System.out.print("Nova Data de Fim (yyyy-MM-dd): ");
                    String novaDataFim = scanner.nextLine();

                    // Atualizar os dados
                    fichaParaAtualizar.setDataInicio(novaDataInicio);
                    fichaParaAtualizar.setDataFim(novaDataFim);

                    fichaTreinoDAO.atualizarFichaTreino(fichaParaAtualizar);
            ...
            case 4:
                // Remover Ficha de Treino
                System.out.print("Digite o ID da Ficha de Treino para remover: ");
                int idFichaRemover = scanner.nextInt();
                scanner.nextLine();  // Limpar o buffer

                fichaTreinoDAO.excluirFichaTreino(idFichaRemover);
            ...
            case 5:
                ...
                List<FichaTreino> fichasTreino = fichaTreinoDAO.listarFichasTreino();

                if (fichasTreino.isEmpty()) {
                    System.out.println("Nenhuma ficha de treino cadastrada.");
                } else {
                    System.out.println("=== Lista de Fichas de Treino ===");
                    System.out.printf("%-5s %-20s %-20s %-15s %-15s%n", "ID", "Membro", "Instrutor", "Início", "Fim");
                    System.out.println("----------------------------------------------------------");
                    for (FichaTreino ft : fichasTreino) {
                        System.out.printf("%-5d %-20s %-20s %-15s %-15s%n", 
                                          ft.getFichaid(), ft.getMembro().getNome(), ft.getInstrutor().getNome(), 
                                          ft.getDataInicio(), ft.getDataFim());
                    }
                    System.out.println("----------------------------------------------------------");
                }
                break;
